...
public class GetEmailByIdTest {
    ...
    public void getEmailByIdTest() {
        ...
        CreateEmailResponse getEmailByIdResponse = given()
                .filter(new AllureRestAssured())
                .auth().basic(api_username, api_password)
                .when()
                .get(BASE_URL + "emails/" + emailId + "/")
                .then()
                .statusCode(200)
                .extract().as(CreateEmailResponse.class);

        Assertions.assertEquals(emailId, getEmailByIdResponse.getId());
    }
}
